...
app.post('/api/v1/itinerary/quote', async (req, res) => {
    // Validar que llegó el cuerpo de la petición
    ...
    const { segments = [], classType = 'ECONOMY' } = req.body;

    if (!Array.isArray(segments) || segments.length === 0) {
        return res.status(400).json({
            success: false,
            error: 'Debes enviar al menos un tramo (segmento) de vuelo'
        });
    }
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        if (!seg.origin || !seg.destination) {
            return res.status(400).json({
                success: false,
                error: `El tramo ${i + 1} debe tener origen y destino`
            });
        }
    }
    ...
    try {
        ...
        let userMessage = 'No se pudo cotizar el itinerario.';
        if (error.message.includes('No se encontró vuelo')) {
            userMessage = error.message;
        } else if (error.message.includes('origen') || error.message.includes('destino')) {
            userMessage = 'Verifica que los aeropuertos de origen y destino sean válidos.';
        }
...
app.get('/api/v1/routes/options', async (req, res) => {
    ...
    try {
        const cheapest = dijkstraService.findCheapestRoute(origin, destination);
        const fastest = dijkstraService.findFastestRoute(origin, destination);

        if (!cheapest.found && !fastest.found) {
            return res.json({
                success: false,
                origin,
                destination,
                error: `No se encontraron rutas disponibles entre ${origin} y ${destination}`,
                suggestion: 'Prueba con otros aeropuertos o fechas diferentes'
            });
        }

        const isSameRoute = cheapest.found && fastest.found &&
            JSON.stringify(cheapest.route) === JSON.stringify(fastest.route);

        res.json({
            success: true,
            origin,
            destination,
            cheapest: cheapest.found ? cheapest : null,
            fastest: fastest.found && !isSameRoute ? fastest : null
        });
    } catch (error) {
        console.error('[Routes Options] Error:', error);
        res.status(500).json({
            success: false,
            error: 'Error al calcular las rutas. Por favor, intenta de nuevo.'
        });
    }
});
...
// ======================================================
// ASIENTOS DE UN VUELO (con validaciones)
// ======================================================

app.get('/api/v1/flights/:flightId/seats', async (req, res) => {
    const { flightId } = req.params;

    // Validación: flightId debe ser un número
    if (!flightId || isNaN(parseInt(flightId))) {
        return res.status(400).json({
            success: false,
            error: 'flightId es requerido y debe ser un número válido'
        });
    }
    ...
    try {
        ...
        // Validación: El vuelo debe existir
        if (flightResult.rows.length === 0) {
            return res.status(404).json({
                success: false,
                error: `No se encontró el vuelo con ID ${flightIdNum}`
            });
        }

        const flight = flightResult.rows[0];
        const columns = ['A', 'B', 'C', 'D', 'E', 'F'];
        const firstClassSeats = flight.first_class_seats || 12;
        const economySeats = flight.economy_seats || 120;

        const firstClassRows = Math.max(1, Math.ceil(firstClassSeats / columns.length));
        const economyRows = Math.max(1, Math.ceil(economySeats / columns.length));

        const SeatState = require('./models/mongodb/SeatState.model');

        let seatStates = [];
        try {
            seatStates = await SeatState.find({ flight_id: flightIdNum });
        } catch (err) {
            console.error('[Seats] Error en MongoDB:', err.message);
        ...
        // Si no hay asientos, devolver array vacío pero no error
        if (seats.length === 0) {
            return res.json({
                success: true,
                flightId: flightIdNum,
                seats: [],
                message: 'No hay asientos configurados para este vuelo'
            });
        }
        ...
        res.status(500).json({
            success: false,
            error: 'Error interno al cargar los asientos. Por favor, intenta de nuevo.'
        });
    }
});
...
app.use((req, res) => {
    res.status(404).json({
        ...
        suggestion: 'Verifica la URL o el método HTTP utilizado'
    });
});
